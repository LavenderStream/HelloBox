...
public class Linear extends BaseShape
{
	...
	private int SSCOPEY = Utils.SCOPEY - 100;
	private int color = Color.parseColor("#eb6982");
	...
	public Linear(Context context, int x , int y)
	{
		super(context);
		this.currentx = x;
		this.currenty = y;
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		case MotionEvent.ACTION_UP:
			...
			// 正方形不能放在最后
			for (int j = 0; j < Utils.MATRIXNUM - 3; j++)
			{
				for (int i = 0; i < Utils.MATRIXNUM; i++)
				{
							...
							&& Math.abs(coorY - y) <= Utils.ACCURACY)
					{
								...
								&& BackGroundView.myTextView[j+3][i].getIsEx() == Utils.DISFILL)
						{
							...
							b.setBackgroundResource(R.drawable.linear_done);
							BackGroundView.myTextView[j+1][i]
									.setBackgroundResource(R.drawable.linear_done);
