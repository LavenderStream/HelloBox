...
public class Matts extends BaseShape
{
	private int wight  = Utils.WIGHT;
	private int currentx = Utils.INITCOOTX;
	private int currenty = Utils.INITCOOTY;
	private int oldCurrentx = Utils.INITCOOTX;
	private int oldCurrenty = Utils.INITCOOTY;
	private int color = Color.parseColor("#96DC53");

	private final int mLimiteLastLine = 1;
	private final int mLimiteLastRow = 1;
	boolean flagCoor = false;
	...
	public Matts(Context context)
	{
		super(context);
	}
	
	public Matts(Context context, AttributeSet attributeSet)
	{
		super(context, attributeSet);
	}


	public Matts(Context context, int x , int y)
	{
		super(context);
		this.currentx = x;
		this.currenty = y;
				...
				for (int i = 0; i < Utils.MATRIXNUM - mLimiteLastRow; i++)
				{
							...
							&& Math.abs(coorY - y) <= Utils.ACCURACY)
					{
								...
								&& BackGroundView.myTextView[j + 1][i + 1].getIsEx() == Utils.DISFILL)
						{
							...
							b.setBackgroundResource(R.drawable.button_done);
							BackGroundView.myTextView[j][i + 1]
									.setBackgroundResource(R.drawable.button_done);
