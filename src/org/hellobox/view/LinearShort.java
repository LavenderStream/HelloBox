...
public class LinearShort extends BaseShape
{
	private int wight  = Utils.WIGHT;
	public int currentx = Utils.INITCOOTX + 20;
	public int currenty = Utils.INITCOOTY - 50;
	public int oldCurrentx = Utils.INITCOOTX + 20;
	public int oldCurrenty = Utils.INITCOOTY -50;
	private int SSCOPEX = Utils.SCOPEX ;
	private int SSCOPEY = Utils.SCOPEY;
	private int color = Color.parseColor("#ffc33c");
	...
	public LinearShort(Context context)
	{
		super(context);
	}
	
	public LinearShort(Context context, AttributeSet attributeSet)
	{
		super(context, attributeSet);
	}


	public LinearShort(Context context, int x , int y)
	{
		super(context);
		this.currentx = x;
		this.currenty = y;
				...
				for (int i = 0; i < Utils.MATRIXNUM; i++)
				{
							...
							&& Math.abs(coorY - y) <= Utils.ACCURACY)
					{
								...
								&& BackGroundView.myTextView[j+1][i].getIsEx() == Utils.DISFILL)
						{
							...
							b.setBackgroundResource(R.drawable.linear_short_done);
							BackGroundView.myTextView[j+1][i]
									.setBackgroundResource(R.drawable.linear_short_done);
