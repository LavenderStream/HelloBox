...
public class SquareLU extends BaseShape
{
	...
	public SquareLU(Context context)
	{
		super(context);
	}
	
	public SquareLU(Context context, AttributeSet attributeSet)
	{
		super(context, attributeSet);
	}


	public SquareLU(Context context, int x , int y)
	{
		super(context);
		this.currentx = x;
		this.currenty = y;
			...
			for (int j = 0; j < Utils.MATRIXNUM - 1; j++)
			{
				for (int i = 0; i < Utils.MATRIXNUM - 1; i++)
				{
							...
							&& Math.abs(coorY - y) <= Utils.ACCURACY)
					{
						...
						if (BackGroundView.myTextView[j][i + 1].getIsEx() == Utils.DISFILL
